...
time, and answer real questions like: is our cost per meal improving over
time?
...
- **Tracks food vs. non-food expenses** separately, with monthly breakdowns and category totals
- **Generates text reports** — full meal log, cost log, monthly trends, donor totals, expense log
- **Generates charts**:
...
- **Python** — core logic
- **SQLite** (`sqlite3`) — local database with four tables: `meals`, `donors`, `store_donations`, `expenses`
...
- `sample_seed_data.py` — example script showing how to log meals, donors, and expenses (uses placeholder data; my real program data is kept private since it includes real donor names and is excluded via `.gitignore`)
